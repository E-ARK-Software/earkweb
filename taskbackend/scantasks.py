...
class WireItLanguageModules(object):
    ...
    def register_language_modules(self):
        ...
        """
        module_list = []
        for module_name, module_params in self.task_modules.items():
            input_params = ""
            exp = 0; sxs = 0; err = 0
            ...
            if WorkflowModules.objects.filter(identifier=module_name).count() == 1:
                ...
                if existing_wf_module.ordval != ord or existing_wf_module.ttype != typ or existing_wf_module.tstage != stg:
                    ...
                    print("Module parameters updated: %s (ordval=%d, ttype=%d, tstage=%d)" % (
                    existing_wf_module.identifier, ord, typ, stg))
            else:
        ...
        for wf_mod in all_wf_modules:
            if wf_mod.identifier not in module_list:
                ...
                for ip in ips_removed_task:
                    print("- IP with process id '%s' (%s) with last task '%s' set to 'DefaultTask'" % (
                    ip.process_id, ip.package_name, ip.last_task))
...
def main():
    ...
    for module_name, module_params in task_modules.items():
