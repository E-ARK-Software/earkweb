...
CAS_HOSTNAME=earkdev.ait.ac.at:8443
CAS_USERNAME=
CAS_PASSWORD=
MYSQL_USERNAME=
MYSQL_PASSWORD=
...
# Build URl for a get request
DEST=http://localhost:8000/earkweb/sipcreator/initialize/test_pack3
...
DEST=http://localhost:8000/earkweb/sipcreator/add_representation
GET_RESULT=`curl -s -X POST -k -L --data "representation=rep_001" -b $COOKIE_JAR -c $COOKIE_JAR "$DEST/$SIP_ID/?$ENCODED_TICKET"`
echo "$GET_RESULT" > result.html
cat ./result.html

# Find test_pack3 and get uuid
DEST=http://localhost:8000/earkweb/sipcreator/uuid
...
DEST=http://localhost:8000/earkweb/sipcreator/ins_file
...
# Update status in DB
echo "update earkcore_informationpackage set last_task_id=\"SIPClose\" where uuid=\"$SIP_UUID\";" | mysql -u $MYSQL_USERNAME -p$MYSQL_PASSWORD eark
