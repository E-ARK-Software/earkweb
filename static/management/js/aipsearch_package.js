...
function callback(data) {
  var repo_item_access_endpoint = 'http://'+django_service_host+':'+django_service_port+'/earkweb/earkweb/access_aip_item/';

  //var lilyNamespace = 'org.eu.earkweb';
  ...
  for (doc of data.response.docs) {
    ...
    var fileAdress = repo_item_access_endpoint + doc[identifier_field] + "/" + doc[titleField]; // encodeURIComponent(doc['lily.id']) + '/field/n$' + blobField + '/data?ns.n=' + lilyNamespace;
...
function askSolr(start) {
  //var solrEndpoint = 'http://'+local_solr_server_ip+':'+local_solr_port+'/solr/storagecore1/';
  var solrEndpoint = 'http://'+django_service_host+':'+django_service_port+'/earkweb/earkweb/solrif/storagecore1/';
